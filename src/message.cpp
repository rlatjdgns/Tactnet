...
MessageType Message::get_message_type(){ return msgType; }
...
void Message::print() const {
    ...
    switch (msgType) {
        ...
        case MessageType::ERROR:
            std::cout << "*** ENVIRONMENTAL ALERT ***\n";
            if (std::stof(values[0]) > 35.0f) std::cout << "      Temperature too hot: "  << values[0] << " °C\n";
            else if (std::stof(values[0]) < 10.0f) std::cout << "      Temperature too cold: " << values[0] << " °C\n";
            if (std::stof(values[1]) > 90.0f) std::cout << "      Humidity too high: "    << values[1] << " %\n";
            std::cout << "      Pressure: " << std::fixed << std::setprecision(2) << std::stof(values[2]) << " hPa\n";
            break;
