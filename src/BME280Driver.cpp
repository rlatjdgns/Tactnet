...
bool BME280Driver::begin() {
    ...
    // set normal mode with oversampling x1
...
void BME280Driver::read_calibration() {
    // temperature and pressure calibration (0x88-0x9F)
    uint8_t reg = 0x88;
    write(fd, &reg, 1);
    uint8_t calib[24];
    ::read(fd, calib, 24);
...
SensorReadings BME280Driver::read() {
    // read raw sensor data from 0xF7
    uint8_t reg = 0xF7;
    write(fd, &reg, 1);
    uint8_t data[8];
    ::read(fd, data, 8);
    ...
    double var2 = (raw_temperature / 131072.0 - dig_T1 / 8192.0) *
